...
T6 = {
    ...
    "parsed_none": ("❌ Impossible de lire le fichier. Vérifiez « Period : ... » et les lignes « No : ... Name : ... ».",
                    "❌ Impossibile leggere il file. Verifica « Period : ... » e le righe « No : ... Name : ... ».",
                    "❌ Cannot read the file. Check “Period : ...” and the “No : ... Name : ...” lines."),
    ...
    "anom_hint": ("DA_RIVEDERE = pointage incomplet (exclu du calcul jusqu'à correction). Modifiez puis « Enregistrer tout ».",
                  "DA_RIVEDERE = timbratura incompleta (esclusa dal calcolo finché non corretta). Modifica poi « Salva tutto ».",
                  "DA_RIVEDERE = incomplete punch (excluded until fixed). Edit then “Save all”."),
    ...
    "dar_warn": ("ligne(s) DA_RIVEDERE exclue(s) du calcul — corrigez-les dans Anomalies puis recalculez",
                 "riga/e DA_RIVEDERE esclusa/e dal calcolo — correggile in Anomalie poi ricalcola",
                 "DA_RIVEDERE row(s) excluded — fix them in Anomalies then recalculate"),
    ...
    "netto_hint": ("Le net final tient compte des prix saisis ci-dessus à la confirmation.",
                   "Il netto finale tiene conto dei premi inseriti qui sopra alla conferma.",
                   "Final net includes the bonuses entered above at confirmation."),
...
FESTIVI_DEFAULT = {
    "01/01/2026": "Nouvel An", "04/04/2026": "Fête de l'Indépendance",
    "01/05/2026": "Fête du Travail", "15/08/2026": "Assomption",
    "01/11/2026": "Toussaint", "25/12/2026": "Noël",
    "20/03/2026": "Korité (indicatif)", "27/05/2026": "Tabaski (indicatif)",
    "26/06/2026": "Tamkharit (indicatif)", "26/08/2026": "Maouloud (indicatif)",
}
...
# =====================================================================
# PARSER (testo incollato E matrice xls)
# =====================================================================
...
def _build_blocchi(rows, get_name):
    ...
    for r in rows:
        ...
        if m:
            ...
            break
    if g2 is not None and g2 < (g1 or 1):
        g2 = g1
    blocchi = []
    i, n = 0, len(rows)
    while i < n:
                ...
                if not _extract_day_map(rows[j]):
                    for ci, cv in enumerate(rows[j]):
                        for t in _cell_times(cv):
                            g = day_map.get(ci)
                            if g is None:
                                g = ci + 1 if 1 <= ci + 1 <= 31 else None
                            if g:
                                per_giorno.setdefault(int(g), []).append(t)
                j += 1
...
def sezione_anomalie(A, lingua):
    ...
    if st.button("💾 " + t6("anom_save_all", lingua), type="primary", use_container_width=True):
        ...
        for i, p in righe[:60]:
            ...
            if (n_stato == orig_stato) and (n_usc == orig_usc) and (n_nota == orig_nota):
                continue
