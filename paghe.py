# -*- coding: utf-8 -*-
"""PROACIER HRM – FASE 7 (Pagina 2) : Présences & Paies [v08.02]
✅ v08.02: STATO LAVORATIVO (prova/assunto/esterno/dimissionario/licenziato; vuoto=prova)
   - esclusione cessati/esterni dal calcolo (switch CONFIG paghe_esterni_attive, default NO)
   - SOLDE DE TOUT COMPTE (préavis, congedi non goduti, indennità licenziamento) + archiviazione
   - RELEVÉ D'HEURES esterni (CSV per fatturazione società terza)
✅ v08.01: paga fissa mensile senza punatura (DIPENDENTI.paga_fissa=SI)
✅ v08.00: trattenute legali Sénégal (CSS/IPRES/IPM/IR) da CONFIG
✅ v07.13: import in memoria, anomalie, paghe quindicina, acconti, buste PDF
Richiede: Apps Script v6.1 + fpdf2 + xlrd. NOTE LEGALI: aliquote/plafond da far
validare dal consulente di Thiès; modificabili da CONFIG senza codice."""
import re, math, random, calendar, csv, io
import requests
from datetime import datetime, date
from fpdf import FPDF
import streamlit as st
VERSIONE_PAGHE = "08.02"
...
_FLOAT_KEYS = ("straordinario_1_percent","straordinario_2_percent","ore_normali_giorno","ritardo_tolleranza_min","assenza_penale_percent","css_lavoratore_percent","css_plafond_mensile","ipres_t1_lav_percent","ipres_t1_plafond","ipres_t2_lav_percent","ipres_t2_plafond","ipres_t3_lav_percent","ipres_t3_plafond","ipm_lavoratore_percent","ipm_plafond_mensile","ir_frais_prof_percent","ir_frais_prof_plafond_mensile","css_af_dat_percent","css_at_mp_dat_percent","fnp_dat_percent","indennita_licenziamento_mesi_per_anno","preavviso_mesi")
def leggi_config(A):
    ...
    for ri in range(sh.nrows):
        ...
        for ci in range(sh.ncols):
            ...
            if isinstance(v, float) and 0 < v < 1:
                hh, mm, ss = xlrd.xld_as_time(v); cells.append(f"{hh:02d}:{mm:02d}")
...
def genera_busta_paga(A, lingua, dip, det, pago, storico, acconti):
    ...
    pdf.set_xy(60, 10); pdf.set_font("Helvetica", "B", 11); pdf.cell(0, 6, az.get("nome",""), 0, 1, "R")
    pdf.set_font("Helvetica", "", 7); pdf.set_xy(60, 17); pdf.multi_cell(130, 4, az.get("indirizzo",""), align="R")
    pdf.set_xy(60, 26); pdf.cell(0, 4, f"tel. {az.get('tel','')} - {az.get('email','')}", 0, 1, "R")
    pdf.set_xy(10, 40); pdf.set_font("Helvetica", "B", 13)
    pdf.cell(0, 8, t6("buste_title", lingua).replace("🖨️","").upper(), 0, 1, "C")
    pdf.set_font("Helvetica", "", 9)
    pdf.cell(0, 6, f"{t6('buste_worker', lingua)}: {A.s_str(dip.get('cognome'))} {A.s_str(dip.get('nome'))} — {A.s_str(dip.get('codice'))}", 0, 1, "L")
    ...
    if det and det.get("fissa"):
        pdf.set_font("Helvetica", "I", 8); pdf.cell(0, 5, t6("fissa_badge", lingua), 0, 1, "L"); pdf.set_font("Helvetica", "", 9)
    if det and det.get("esterno"):
        pdf.set_font("Helvetica", "I", 8); pdf.cell(0, 5, t6("esterno_badge", lingua), 0, 1, "L"); pdf.set_font("Helvetica", "", 9)
    pdf.ln(2)
    pdf.set_font("Helvetica", "B", 9)
    for lab in ("col_giorni","col_ore","col_stra","col_rit"): pdf.cell(47, 6, t6(lab, lingua), 1, 0, "C")
    pdf.ln()
    pdf.set_font("Helvetica", "", 9)
    ...
    if det and det.get("tratt") and det["tratt"].get("totale", 0) > 0:
        ...
        pdf.set_font("Helvetica", "B", 9); pdf.cell(0, 6, t6("tratt_title", lingua), 0, 1, "L")
        pdf.set_font("Helvetica", "", 8)
        for k, lab in (("css","tratt_css"),("ipres","tratt_ipres"),("ipm","tratt_ipm"),("ir","tratt_ir")):
            pdf.cell(0, 5, f"- {t6(lab, lingua)}: {tratt[k]:,.0f} FCFA", 0, 1, "L")
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(0, 6, f"{t6('tratt_tot', lingua)}: -{tratt['totale']:,.0f} FCFA", 0, 1, "L")
        pdf.set_font("Helvetica", "I", 7); pdf.multi_cell(0, 4, t6("tratt_note", lingua)); pdf.set_font("Helvetica", "", 9)
    ...
    if tratt_vis and det:
        ...
        pdf.set_font("Helvetica", "B", 8); pdf.cell(0, 5, t6("dat_title", lingua), 0, 1, "L")
        pdf.set_font("Helvetica", "", 7)
        for k, lab in (("dat_css_af","dat_css_af"),("dat_css_at","dat_css_at"),("dat_ipres","dat_ipres"),("dat_ipm","dat_ipm"),("dat_fnp","dat_fnp")):
            pdf.cell(0, 4, f"- {t6(lab, lingua)}: {tratt[k]:,.0f} FCFA", 0, 1, "L")
    if acconti:
        pdf.ln(2); pdf.set_font("Helvetica", "B", 9); pdf.cell(0, 6, t6("buste_avances", lingua), 0, 1, "L")
        pdf.set_font("Helvetica", "", 8)
        for a in acconti:
            ...
            pdf.cell(0, 5, f"- {A.s_str(a.get('tipo_acconto')) or 'generico'}: {to_float(a.get('importo')):,.0f} FCFA{piano}", 0, 1, "L")
    if storico:
        pdf.ln(2); pdf.set_font("Helvetica", "B", 9); pdf.cell(0, 6, t6("buste_hist", lingua), 0, 1, "L")
        pdf.set_font("Helvetica", "", 8)
        for p in storico:
            pdf.cell(0, 5, f"- {A.s_str(p.get('periodo_da'))}→{A.s_str(p.get('periodo_a'))}: NET {to_float(p.get('importo_netto')):,.0f} FCFA", 0, 1, "L")
...
def sezione_buste(A, lingua):
    ...
    for d in dips:
        ...
        if cod: lab = f"{cod} — {A.s_str(d.get('cognome'))} {A.s_str(d.get('nome'))}"; opzioni.append(lab); codmap[lab] = cod
...
def sezione_anomalie(A, lingua):
    ...
    if st.button("💾 " + t6("anom_save_all", lingua), type="primary", use_container_width=True):
        ...
        for i, p in righe[:60]:
            ...
            n_s = st.session_state.get(f"f6s_{i}", o_s); n_u = (st.session_state.get(f"f6u_{i}") or "").strip(); n_n = st.session_state.get(f"f6n_{i}", o_n)
...
pagina_fase6 = pagina_fase7  # retrocompat v20.x
