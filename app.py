...
def genera_pdf_lavoratore(d, lingua="fr"):
    ...
    pdf.add_page()
    pdf.set_left_margin(7); pdf.set_right_margin(7)
    pdf.set_font("Helvetica", "B", 11); pdf.set_x(7); pdf.cell(0, 7, "CONSENTEMENT AU TRAITEMENT DES DONNÉES PERSONNELLES", 0, 1, "C")
    pdf.ln(10)
    pdf.set_font("Helvetica", "", 8)
    ...
    for ln_ in _pdf_safe(testo_legale("reglement_interieur", reglement_testo(az))).split("\n"):
        pdf.set_x(20); pdf.multi_cell(0, 4.6, ln_)
